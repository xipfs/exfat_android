...
public class UsbMassStorageDevice{
    ...
    public void setupDevice(){
        ...
        try {
            ScsiBlockDevice scsiBlockDevice = BlockDeviceDriverFactory.createBlockDevice(communication,maxLun[0]);
            ByteBlockDevice byteBlockDevice  = new ByteBlockDevice(scsiBlockDevice);
            // 获取分区表
            PartitionTable partitionTable = PartitionTableFactory.createPartitionTable(byteBlockDevice);
            // 初始化分区信息
            initPartitions(partitionTable, scsiBlockDevice);
        } catch (IOException e) {
            e.printStackTrace();
        }

    }
    private void initPartitions(PartitionTable partitionTable,BlockDeviceDriver blockDevice){

        List<PartitionTableEntry> entrys = partitionTable.getPartitionTableEntries();
        for(PartitionTableEntry entry : entrys){
            Partition.createPartition(entry, blockDevice);
        }
    }
